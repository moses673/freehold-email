...
const adminHash = 'SHA256:' + createHash('sha256').update('freehold2026').digest('hex');
db.run(
  `INSERT OR IGNORE INTO users (email, password_hash, name) VALUES (?, ?, ?)`,
  ['admin@freehold.local', adminHash, 'Admin']
);

const adminUser = get(`SELECT id FROM users WHERE email = 'admin@freehold.local'`);
const userId = adminUser ? adminUser.id : 1;

// Seed default lists with user_id
db.run(`INSERT OR IGNORE INTO lists (user_id, name, description) VALUES (?, 'Subscribers', 'General mailing list')`, [userId]);
db.run(`INSERT OR IGNORE INTO lists (user_id, name, description) VALUES (?, 'VIP Customers', 'High-value customers')`, [userId]);
db.run(`INSERT OR IGNORE INTO lists (user_id, name, description) VALUES (?, 'Trial Users', 'Free trial signups')`, [userId]);

const subscribersList = get(`SELECT id FROM lists WHERE user_id = ? AND name = 'Subscribers'`, [userId]);
const vipList = get(`SELECT id FROM lists WHERE user_id = ? AND name = 'VIP Customers'`, [userId]);
const subscribersListId = subscribersList ? subscribersList.id : 1;
const vipListId = vipList ? vipList.id : 2;

// Seed sample contacts with user_id
db.run(`INSERT OR IGNORE INTO contacts (user_id, email, first_name, last_name, list_id, tags) VALUES (?, 'alice@example.com', 'Alice', 'Johnson', ?, '["active"]')`, [userId, subscribersListId]);
db.run(`INSERT OR IGNORE INTO contacts (user_id, email, first_name, last_name, list_id, tags) VALUES (?, 'bob@example.com', 'Bob', 'Smith', ?, '["active"]')`, [userId, subscribersListId]);
db.run(`INSERT OR IGNORE INTO contacts (user_id, email, first_name, last_name, list_id, tags) VALUES (?, 'carol@example.com', 'Carol', 'White', ?, '["vip","premium"]')`, [userId, vipListId]);
db.run(`INSERT OR IGNORE INTO contacts (user_id, email, first_name, last_name, list_id, tags) VALUES (?, 'dave@example.com', 'Dave', 'Brown', ?, '["vip","engaged"]')`, [userId, vipListId]);
db.run(`INSERT OR IGNORE INTO contacts (user_id, email, first_name, last_name, list_id, tags) VALUES (?, 'eve@example.com', 'Eve', 'Davis', ?, '["inactive"]')`, [userId, subscribersListId]);

// Seed email templates (as presets with user_id = NULL)
const templates = [
...
for (const [name, category, subject, html] of templates) {
  db.run(
    `INSERT OR IGNORE INTO templates (user_id, name, category, subject, html_content, is_preset) VALUES (?, ?, ?, ?, ?, 1)`,
    [null, name, category, subject, html]
  );
}
